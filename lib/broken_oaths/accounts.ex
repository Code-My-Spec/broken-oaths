defmodule BrokenOaths.Accounts do
  ...
  def subscribe_account(%Scope{} = scope) do
    key = scope.user.id
    Phoenix.PubSub.subscribe(BrokenOaths.PubSub, "user:#{key}:account")
  end

  def subscribe_member(%Scope{} = scope) do
    key = scope.user.id
    Phoenix.PubSub.subscribe(BrokenOaths.PubSub, "user:#{key}:member")
  end

  defp broadcast_account(%Scope{} = scope, message) do
    key = scope.user.id
    Phoenix.PubSub.broadcast(BrokenOaths.PubSub, "user:#{key}:account", message)
  end

  defp broadcast_member(%Scope{} = scope, message) do
    key = scope.user.id
    Phoenix.PubSub.broadcast(BrokenOaths.PubSub, "user:#{key}:member", message)
  end
  ...
  def subscribe_invitations(%Scope{} = scope) do
    key = scope.user.id
    Phoenix.PubSub.subscribe(BrokenOaths.PubSub, "user:#{key}:invitations")
  end

  defp broadcast_invitation(%Scope{} = scope, message) do
    key = scope.user.id
    Phoenix.PubSub.broadcast(BrokenOaths.PubSub, "user:#{key}:invitations", message)
  end
  ...
  defp send_invitation_email(invitation) do
    url = BrokenOathsWeb.Endpoint.url() <> "/invitations/accept/#{invitation.token}"
