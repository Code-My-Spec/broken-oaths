defmodule BrokenOaths.Feudal.Rebellion do
  @moduledoc """
  ...
  Mirrors `BrokenOaths.Feudal.Vassalage`'s own split with `BrokenOaths.
  Game.Vassalization`: this module owns the schema, `t()`, and the
  changeset (invalid states impossible); its sibling
  `BrokenOaths.Feudal.Rebellion.Resolution`, in this same file per this
...
defmodule BrokenOaths.Feudal.Rebellion.Resolution do
  @moduledoc """
  PURE resolution math for `BrokenOaths.Feudal.Rebellion` (stories
  ...
  >= city_resistance(seed, tile_id)`:
    ...
    * `tyranny_score/2` — 0..100, INCREASING as the lord's Honor falls
      ...
      UNCLAMPED elsewhere in this codebase (`BrokenOaths.Feudal.Tribute`'s
  ...
  `BrokenOaths.Feudal.OathStrain.rebellion_army_size/1` (story 913) —
