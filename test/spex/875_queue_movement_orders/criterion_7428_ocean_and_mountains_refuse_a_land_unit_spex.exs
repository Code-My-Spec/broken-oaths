defmodule BrokenOathsSpex.Story875.Criterion7428Spex do
  ...
  spex "ocean and mountains refuse a land unit" do
    scenario "an impassable destination queues nothing" do
      ...
      given_ "a deep-ocean tile is known", context do
        %{deep_ocean: deep_ocean} = Fixtures.region_partition(context.world)

        [unit | _] =
          for u <- Fixtures.player_units(context.world, context.user), u.type == :lord, do: u

        {:ok, context |> Map.put(:unit, unit) |> Map.put(:ocean_tile, Enum.at(Enum.to_list(deep_ocean), 0))}
      end

      when_ "the player targets the ocean tile", context do
        render_hook(context.play_live, "queue_move", %{
          "unit_id" => context.unit.id,
          "to_tile" => context.ocean_tile
        })
